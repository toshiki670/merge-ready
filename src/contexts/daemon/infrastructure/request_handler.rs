...
mod tests {
    ...
    #[test]
    fn process_query_fresh_cache_returns_output_without_scheduling_refresh() {
        let repo_id = RepoId::new("repo");
        let mut entries = HashMap::new();
        entries.insert(repo_id.clone(), make_entry("✓ Ready", RefreshMode::Warm));
        ...
        assert!(matches!(result.response, Response::Output { ref output } if output == "✓ Ready"));
        assert!(result.refresh_repo_id.is_none());
        assert!(!result.restart_after_response);
    }

    #[test]
    fn process_query_stale_not_refreshing_schedules_refresh() {
        let repo_id = RepoId::new("repo");
        let mut entries = HashMap::new();
        entries.insert(
            repo_id.clone(),
            make_stale_entry("✓ Ready", RefreshMode::Warm, 9999),
        );
        let result = process_query(
            &repo_id,
            "main".to_owned(),
        ...
        assert!(result.refresh_repo_id.is_some());
        assert!(entries[&repo_id].is_refreshing());
    }

    #[test]
    fn process_query_stale_refreshing_fetched_returns_cached_output() {
        let repo_id = RepoId::new("repo");
        let mut entry = make_stale_entry("✓ Ready", RefreshMode::Warm, 9999);
    ...
    #[test]
    fn process_query_stale_refreshing_not_fetched_returns_loading() {
        let repo_id = RepoId::new("repo");
        // CacheEntry::new() → Loading 状態（has_fetched=false, is_refreshing=true, fetched_at=past）
        let mut entries = HashMap::new();
        entries.insert(
            repo_id.clone(),
            CacheEntry::new(PathBuf::new(), "main".to_owned(), 5),
        );
        let result = process_query(
            &repo_id,
            "main".to_owned(),
        ...
        assert!(
            matches!(result.response, Response::Output { ref output } if output == "? loading")
        );
        assert!(result.refresh_repo_id.is_none());
    }

    #[test]
    fn process_query_cold_entry_resets_cold_count() {
        let repo_id = RepoId::new("repo");
        let mut entry = make_stale_entry("✓ Ready", RefreshMode::Warm, 9999);
        ...
        assert_eq!(entries[&repo_id].cold_refresh_count(), 0);
    }

    #[test]
    fn process_query_terminal_stale_resets_mode_to_warm() {
        let repo_id = RepoId::new("repo");
        let mut entries = HashMap::new();
        entries.insert(
            repo_id.clone(),
            make_stale_entry("", RefreshMode::Terminal, 9999),
        );
        process_query(
            &repo_id,
            "main".to_owned(),
    ...
    #[test]
    fn process_query_initial_miss_creates_entry_and_schedules_refresh() {
        let repo_id = RepoId::new("new-repo");
        let mut entries = HashMap::new();
        let result = process_query(
            &repo_id,
            "main".to_owned(),
